...
Bun.serve({
  ...
  async fetch(req) {
    ...
    // Static file serving (if not an API route handled above)
      ...
      if (requestedPath === '/') {
        filePath = path.join(publicDir, 'html', 'index.html');
      } else if (htmlFilesInHtmlDir.includes(requestedPath)) {
        filePath = path.join(publicDir, 'html', requestedPath.substring(1));
      } else {
        // For other files like /style.css, /js/app.js, /assets/images/logo.png
        filePath = path.join(publicDir, requestedPath.startsWith('/') ? requestedPath.substring(1) : requestedPath);
      }
