...
describe('Application and Company Dashboard Tests', () => {
  ...
  beforeEach(async () => {
    ...
    console.log('[tests/application.test.js] Created test data via API.');
    ...
    if (window && window.localStorage) {
        ...
        // If window/localStorage is not yet available (e.g., before JSDOM is created in a test)
   ...
   test('Student can view their submitted applications', async () => {
    ...
    const studentApplicationsFetchMock = mock(async (url, options) => {
        ...
        if (parsedUrl.pathname === '/applications' && parsedUrl.searchParams.get('studentId') === String(student.id)) {
            ...
            return Promise.resolve(new window.Response(JSON.stringify(applications), { status: 200, headers: { 'Content-Type': 'application/json' } }));
        }
  ...
  test('Student can submit application with notes and files', async () => {
    ...
    const transcriptFile = new window.File(['transcript content'], 'transcript.pdf', { type: 'application/pdf' });
    const enrollmentProofFile = new window.File(['enrollment content'], 'enrollment.docx', { type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' });
    const otherFile1 = new window.File(['other file 1 content'], 'resume.txt', { type: 'text/plain' });
    const otherFile2 = new window.File(['other file 2 content'], 'portfolio.jpg', { type: 'image/jpeg' });
    ...
    // Mock the fetch call for application submission
    const applicationPostMock = mock(async (url, options) => {
        if (url.toString().endsWith('/api/applications') && options?.method === 'POST') {
            ...
            expect(transcriptEntry).toBeInstanceOf(window.File);
            ...
            expect(enrollmentProofEntry).toBeInstanceOf(window.File);
            ...
            expect(otherFilesEntries[0]).toBeInstanceOf(window.File);
            expect(otherFilesEntries[0].name).toBe('resume.txt');
            expect(otherFilesEntries[0].type).toBe('text/plain');
            expect(otherFilesEntries[1]).toBeInstanceOf(window.File);
        ...
        // Fallback to original fetch for other calls (e.g., fetching opportunity details)
        return originalFetch(url, options);
    });
    global.fetch = applicationPostMock; // Temporarily override global fetch
    ...
    // Assertions
    expect(applicationPostMock).toHaveBeenCalledTimes(1); // Verify the POST request was made
   ...
   test('Company user can view applications and file links', async () => {
    ...
    const companyDashboardFetchMock = mock(async (url, options) => {
        ...
        if (parsedUrl.pathname === '/api/users' && parsedUrl.searchParams.get('companyId') === String(company.id)) {
             ...
             return Promise.resolve(new window.Response(JSON.stringify(company), { status: 200 }));
        }
        if (parsedUrl.pathname === '/api/opportunities' && parsedUrl.searchParams.get('companyId') === String(company.id)) {
             ...
             return Promise.resolve(new window.Response(JSON.stringify(companyOpportunities), { status: 200 }));
        }
        ...
        if (parsedUrl.pathname.startsWith('/api/applications/') && parsedUrl.pathname.endsWith('/files')) {
             ...
             actualServerDb.close();
             return Promise.resolve(new window.Response(JSON.stringify(files), { status: 200, headers: { 'Content-Type': 'application/json' } }));
        }
    ...
    // Verify that file links are displayed for the test application
    const fileLinks = testApplicationItem.querySelectorAll('.application-item a');
    expect(fileLinks.length).toBe(2); // Expecting 2 file links based on mock data

    // Verify the href and text content of the file links
    expect(fileLinks[0].textContent).toBe('test_transcript.pdf');
    // The href should match the hypothetical serving endpoint defined in company-dashboard.js
    expect(fileLinks[0].href).toBe(`${SERVER_URL}/uploads/applications/test_transcript.pdf`); 
    expect(fileLinks[1].textContent).toBe('test_resume.docx');
    expect(fileLinks[1].href).toBe(`${SERVER_URL}/uploads/applications/test_resume.docx`);
  ...
  test('Student can submit application with notes and files', async () => {
   ...
   test('Company user can view applications and file links', async () => {
    ...
    // Verify that file links are displayed for the test application
    // This requires modifying renderReceivedApplications to display file links.
    // For now, we'll check if the application object passed to renderReceivedApplications
    // in the mock contains the 'files' array.
    // A proper test would check the DOM for rendered links.
